...
import { authSessionResponse, loginLocalUser, setAuthSessionCookie } from "@/src/lib/auth";
...
export async function POST(request: Request) {
  ...
  try {
    const user = await loginLocalUser(payload.username ?? "", payload.passphrase ?? "");
    ...
    const response = NextResponse.json({
      ...
      status: {
        ...
        status: "ok",
        detail: "Signed in. Workspace requests are now scoped to this user.",
        url: null
      }
    });
